...
def tweet(match_updates):
    ...
    if match_updates["event"] == "FT":
        tweet = "The final whistle has blown, and that is the end of today's {0} match!\n\n{1} {2} - {3} {4}".format(match_updates.get("competitoin"), match_updates.get("home team"), match_updates.get("home score"), match_updates.get("away team"), match_updates.get("away score"))
        client.create_tweet(text=tweet)
        print("Tweet sent!")
