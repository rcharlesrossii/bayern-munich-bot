...
def tweet(match_updates):
    if match_updates["event"] == 0:
        tweet = "Today's {0} match between {1} and {2} is about to kick-off.\n\nWe should be set for an exciting match!\n\n#{1}Vs{2}".format(match_updates.get("competition"), match_updates.get("home team"), match_updates.get("away team"))
        client.create_tweet(text=tweet)
        print("Tweet sent!")
    
    if match_updates["event"] == "Goal":
        tweet = "GOAL!!!\n\n{0}'s {1} has scored a goal at the {2}' mark!\n\n{3} {4} - {5} {6}\n\n#{3}Vs{5}".format(match_updates.get("team"), match_updates.get("player"), match_updates.get("minute"), match_updates.get("home team"), match_updates.get("home score"), match_updates.get("away team"), match_updates.get("away score"))
        client.create_tweet(text=tweet)
        print("Tweet sent!")

    if match_updates["event"] == "Assist":
        tweet = "{0}'s {1} is credited for assisting the goal at the {2}' mark.\n\n#{3}Vs{4}".format(match_updates.get("team"), match_updates.get("player"), match_updates.get("minute"), match_updates.get("home team"), match_updates.get("away team"))
        client.create_tweet(text=tweet)
        print("Tweet sent!")
    ...
    if match_updates["event"] == "VAR":
        tweet = "What unfortunate luck for {0}!\n\nVAR has decided that {1}'s {0} did not score a goal at the {2}' mark!\n\n#{3}Vs{4}".format(match_updates.get("player"), match_updates.get("team"), match_updates.get("minute"), match_updates.get("home team"), match_updates.get("away team"))
        client.create_tweet(text=tweet)
        print("Tweet sent!")
    ...
    if match_updates["event"] == "FT":
        tweet = "The final whistle has blown, and that is the end of today's {0} match!\n\n{1} {2} - {3} {4}\n\n#{1}Vs{3}".format(match_updates.get("competition"), match_updates.get("home team"), match_updates.get("home score"), match_updates.get("away team"), match_updates.get("away score"))
        client.create_tweet(text=tweet)
        print("Tweet sent!")
