...
def tweet(match_updates):
    if match_updates["event type"] == 0:
        tweet = "Today's {0} match between {1} and {2} is about to kick-off.\n\nWe should be set for an exciting match!\n\n#{3}vs{4} {5}".format(match_updates.get("competition"), match_updates.get("home team"), match_updates.get("away team"), match_updates.get("home team code"), match_updates.get("away team code"), today)
        client.create_tweet(text=tweet)
        print("Tweet sent!")
    
    if match_updates["event type"] == "Goal":
        tweet = "GOAL!!!\n\n{0}'s {1} has scored a goal at the {2}' mark!\n\n{3} {4} - {5} {6}\n\n#{7}vs{8} {9}".format(match_updates.get("team"), match_updates.get("player"), match_updates.get("minute"), match_updates.get("home team"), match_updates.get("home score"), match_updates.get("away team"), match_updates.get("away score"), match_updates.get("home team code"), match_updates.get("away team code"), today)
        client.create_tweet(text=tweet)
        print("Tweet sent!")

    if match_updates["event type"] == "Assist":
        tweet = "{0}'s {1} is credited for assisting the goal at the {2}' mark.\n\n#{3}vs{4} {5}".format(match_updates.get("team"), match_updates.get("player"), match_updates.get("minute"), match_updates.get("home team code"), match_updates.get("away team code"), today)
        client.create_tweet(text=tweet)
        print("Tweet sent!")

    if match_updates["event type"] == "Own Goal":
        if match_updates["team"] == match_updates["home team"]:
            tweet = "Uh Oh...\n\n{0}'s {1} has scored an own goal at the {2}' mark!\n\n{3} {4} - {0} {5}\n\n#{6}vs{7} {8}".format(match_updates.get("away team"), match_updates.get("player"), match_updates.get("minute"), match_updates.get("home team"), match_updates.get("home score"), match_updates.get("away score"), match_updates.get("home team code"), match_updates.get("away team code"), today)
            client.create_tweet(text=tweet)
            print("Tweet sent!")
        if match_updates["team"] == match_updates["away team"]:
            tweet = "Uh Oh...\n\n{0}'s {1} has scored an own goal at the {2}' mark!\n\n{0} {3} - {4} {5}\n\n#{6}vs{7} {8}".format(match_updates.get("home team"), match_updates.get("player"), match_updates.get("minute"), match_updates.get("home score"), match_updates.get("away team"), match_updates.get("away score"), match_updates.get("home team code"), match_updates.get("away team code"), today)
            client.create_tweet(text=tweet)
            print("Tweet sent!")
    ...
    if match_updates["event type"] == "Second Yellow Card":
        tweet = "Goodbye {0}!\n\n{1}'s {0} has received his second yard at the {2}' mark and is forced off the pitch!\n\n{1} will be down a player for the rest of the match.\n\n#{3}vs{4} {5}".format(match_updates.get("player"), match_updates.get("team"), match_updates.get("minute"), match_updates.get("home team code"), match_updates.get("away team code"), today)
        client.create_tweet(text=tweet)
        print("Tweet sent!")
